...
serve(async (req) => {
  ...
  try {
    ...
    const shouldRecordWaiver =
      waiver_minor &&
      parent_guardian_name?.trim() &&
      parent_guardian_email?.trim() &&
      parent_signature_name?.trim() &&
      volunteer_signature_name?.trim()
      ...
      if (existingSignup.status === 'cancelled') {
        await supabaseAdmin
          ...
          .update({
            ...
            waiver_signed: shouldRecordWaiver,
          })
    ...
    if (shouldRecordWaiver) {
      const { data: waiverSettings, error: waiverSettingsError } = await supabaseAdmin
        .from('waiver_settings')
        .select('version, waiver_text')
        .single()
      if (waiverSettingsError || !waiverSettings) {
        console.error('create-signup-for-new-user: waiver_settings lookup failed', waiverSettingsError)
      } else {
        const { error: waiverInsertError } = await supabaseAdmin.from('waivers').insert({
          volunteer_id: user_id,
          signature_name: volunteer_signature_name.trim(),
          waiver_version: waiverSettings.version,
          waiver_text: waiverSettings.waiver_text,
          parent_guardian_name: (parent_guardian_name as string).trim(),
          parent_guardian_email: (parent_guardian_email as string).trim(),
          parent_guardian_phone: parent_guardian_phone != null ? String(parent_guardian_phone).trim() || null : null,
          parent_signature_name: (parent_signature_name as string).trim(),
          parent_signed_at: new Date().toISOString(),
        })
        if (waiverInsertError) {
          console.error('create-signup-for-new-user: waiver insert failed', waiverInsertError)
        }
      }
    }
